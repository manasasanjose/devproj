...
foreach($arr1 as $key => $value)
{
	if($key=="password")
	{
		$sql1="SELECT `password` from `volunteer` where `email`='$email'"; //to check if the password was updated by the user.
	...
	$sql="UPDATE volunteer SET $key = '$value' WHERE email = '$email'";
